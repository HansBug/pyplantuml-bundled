...
### Added
- Pre-built wheels for 8 `(OS, arch, libc)` targets, one wheel per platform, all `py3-none-{platform}` so a single wheel covers Python 3.6 – 3.14:
  - `manylinux_2_17_x86_64`, `manylinux_2_17_aarch64`
  - `musllinux_1_1_x86_64`, `musllinux_1_1_aarch64`
  - `macosx_*_x86_64`, `macosx_*_arm64`
  - `win_amd64`, `win_arm64`
- Bundled `jlink`-stripped Eclipse Temurin 11 JRE (Microsoft OpenJDK 11 on Windows ARM64 — Eclipse Temurin 11 has no `win-aarch64` build).
...
- Same Linux x86_64 wheel passes 12 / 12 pytest across the full Python range — Python 3.6.9 via the `ubuntu:18.04` stage-2 job, and 3.7.17 / 3.8.20 / 3.9.25 / 3.10.20 / 3.11.15 / 3.12.13 / 3.13.13 / 3.14.4 via cibuildwheel's per-Python test step. Confirms one `py3-none-{plat}` wheel really covers the full version range.
